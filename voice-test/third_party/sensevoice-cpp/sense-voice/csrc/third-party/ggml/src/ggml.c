...
struct ggml_tensor * ggml_conv_1d(
        ...
        int                   p0,
        int                   d0) {
    // Use F32 im2col for F32 weights (pure F32 GEMM with SIMD),
    // F16 im2col for F16 weights (F16 GEMM, saves memory bandwidth).
    enum ggml_type im2col_type = (a->type == GGML_TYPE_F16) ? GGML_TYPE_F16 : GGML_TYPE_F32;

...
// Set a custom log callback to redirect SenseVoice's own log output
// (SENSE_VOICE_LOG_* / SENSEVOICE_LOG_* macros). Without this, logs go
// to stderr via sense_voice_log_callback_default, ignoring ggml_log_set
// which only controls ggml's separate log state.
void sense_voice_log_set(ggml_log_callback callback, void * user_data);
